...
def _validate_feeds_config(data: Any):
    ...
    for index, feed_group in enumerate(data):
        ...
        if not isinstance(feed_group["name"], str):
            raise InvalidConfigError(f"Feed group {index}: 'name' must be string")

        if not isinstance(feed_group["urls"], list):
            raise InvalidConfigError(f"Feed group {index}: 'urls' must be a list")
